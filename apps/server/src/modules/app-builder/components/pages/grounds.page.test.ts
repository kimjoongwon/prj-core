...
describe('GroundsPage', () => {
  describe('페이지 기본 구성', () => {
    it('그라운드 리스트 페이지를 올바르게 구성해야 한다', () => {
      const result = getGroundsPage();

      expect(result.name).toBe('그라운드 리스트');
  ...
  describe('DataGridBuilder 테이블 설정', () => {
    ...
    it('올바른 컬럼 구성을 가져야 한다', () => {
      ...
      const expectedColumns = [
        { accessorKey: 'name', headerName: '사업장명' },
        { accessorKey: 'label', headerName: '라벨' },
        { accessorKey: 'address', headerName: '주소' },
        { accessorKey: 'businessNo', headerName: '사업자등록번호' },
        { accessorKey: 'phone', headerName: '전화번호' },
        { accessorKey: 'email', headerName: '이메일' },
