...
// 50명의 일반 유저 데이터
export const userSeedData: UserSeedData[] = Array.from({ length: 50 }, (_, index) => {
...
// 다양한 그라운드 데이터 (50개)
export const groundSeedData: GroundSeedData[] = Array.from({ length: 50 }, (_, index) => {
  const companyNumber = index + 1;
  return {
    name: `회사-${companyNumber}`,
    label: companyNumber <= 10 ? '본점' : companyNumber <= 30 ? '지점' : '분점',
...
export const userGroundMapping: { userIndex: number; groundIndices: number[] }[] = Array.from(
  { length: 50 },
  (_, userIndex) => {
