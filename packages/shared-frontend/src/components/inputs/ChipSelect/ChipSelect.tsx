...
export const ChipSelect = observer(<T extends object>(props: ChipSelectProps<T>) => {
	...
	const handleChipPress = (value: string) => {
		...
		if (selectionMode === 'single') {
			...
			const currentValues = Array.isArray(localState.value) ? localState.value : [];
	...
	const isSelected = (value: string): boolean => {
		...
		if (selectionMode === 'multiple') {
			return Array.isArray(localState.value) && localState.value.includes(value);
		}
		return false;
	};
		...
		<div className="flex flex-wrap gap-2">
			{options.map((option) => (
				<Chip
					...
					onClick={selectionMode !== 'none' ? () => handleChipPress(option) : undefined}
				>
					{option}
				</Chip>
			))}
